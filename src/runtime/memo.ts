...
/** Drop the cache bucket for a single memo id. Called by the worklist when
 *  a chain node is retired: the bucket minted against that node's entry
 *  guards is no longer safe to serve, and any follow-up dispatch will miss
 *  and re-execute at whatever chain survives retirement. */
export function clearMemoId(id: string): void {
  cache.delete(id);
}
