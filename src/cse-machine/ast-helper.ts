...
export const hasReturnStatement = (
  block: es.BlockStatement | StatementSequence,
): boolean => {
  let hasReturn = false;
  for (const statement of block.body) {
    if (isReturnStatement(statement)) {
      ...
      hasReturn =
        hasReturn || hasReturnStatementIf(statement as es.IfStatement);
...
export const hasReturnStatementIf = (statement: es.IfStatement): boolean => {
  ...
  // Parser enforces that if/else have braces (block statement)
  hasReturn =
    hasReturn && hasReturnStatement(statement.consequent as es.BlockStatement);
  if (statement.alternate) {
    if (isIfStatement(statement.alternate)) {
      hasReturn =
        hasReturn &&
        hasReturnStatementIf(statement.alternate as es.IfStatement);
    } else if (
      isBlockStatement(statement.alternate) ||
      isStatementSequence(statement.alternate)
    ) {
...
export const isStatementSequence = (
  node: ControlItem,
): node is StatementSequence => {
