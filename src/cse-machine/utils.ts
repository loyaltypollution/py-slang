...
const propertySetter: PropertySetter = new Map<string, Transformer>([
  ...
  [
    ...
    (item: ControlItem) => {
      ...
      node.isEnvDependent =
        isEnvDependent(node.left) || isEnvDependent(node.right);
      return node;
    },
  ],
  [
  ...
  [
    ...
    (item: ControlItem) => {
      ...
      node.isEnvDependent =
        isEnvDependent(node.property) || isEnvDependent(node.object);
      return node;
    },
  ],
  [
  ...
  [
    ...
    (item: ControlItem) => {
      ...
      node.isEnvDependent =
        ...
        node.arguments.some((arg) => isEnvDependent(arg));
      return node;
    },
  ],
  [
  ...
  [
    ...
    (item: ControlItem) => {
      ...
      node.isEnvDependent =
        isEnvDependent(node.body) || isEnvDependent(node.test);
      return node;
    },
  ],
  [
  ...
  [
    ...
    (item: ControlItem) => {
      ...
      instr.isEnvDependent =
        isEnvDependent(instr.test) || isEnvDependent(instr.body);
      return instr;
    },
  ],
  [
  ...
  [
    ...
    (item: ControlItem) => {
      ...
      instr.isEnvDependent =
        isEnvDependent(instr.consequent) || isEnvDependent(instr.alternate);
      return instr;
    },
  ],
]);
...
function declareVariables(
  ...
  for (const declaration of node.declarations) {
    ...
    declareIdentifier(
      context,
      (declaration.id as es.Identifier).name,
      node,
      environment,
      constant,
    );
...
export const checkNumberOfArguments = (
  ...
  if (callee instanceof Closure) {
    ...
    if (
      hasVarArgs
        ? callee.minArgsNeeded > args.length
        : callee.length !== args.length
    ) {
      // TODO: error
    }
  }
...
export const isSimpleFunction = (node: any) => {
  if (
    ...
    node.body.type !== "StatementSequence"
  ) {
    return true;
  } else {
