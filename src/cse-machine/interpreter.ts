 ...
 */
export function CSEResultPromise(
  context: Context,
  value: Value,
): Promise<Result> {
          ...
          switch (spec.type) {
            case "ImportSpecifier": {
              if (spec.imported.type === "Identifier") {
                ...
                throw new Error(
                  `Unexpected literal import: ${spec.imported.value}`,
                );
              }
              break;
            }
...
export function* generateCSEMachineStateStream(
  ...
  while (command) {
    ...
    if (isNode(command)) {
      ...
      cmdEvaluators[(command as Instr).instrType](
        command,
        context,
        control,
        stash,
...
const cmdEvaluators: { [type: string]: CmdEvaluator } = {
  ...
  VariableDeclaration: function (
    ...
    const declaration: es.VariableDeclarator = (
  ...
  Literal: function (
    ...
    if (literalValue !== undefined) {
      ...
      let value: Value;
      let pyComplexNumber = new PyComplexNumber(
        complexValue.real,
        complexValue.imag,
      );
      try {
  ...
  Identifier: function (
    ...
    if (builtInConstants.has((command as es.Identifier).name)) {
      const builtinCons = builtInConstants.get(
        (command as es.Identifier).name,
      )!;
  ...
  UnaryExpression: function (
    ...
    control.push(
      instr.unOpInstr(
        (command as es.UnaryExpression).operator,
        command as es.UnaryExpression,
      ),
    );
  ...
  BinaryExpression: function (
    ...
    control.push(
      instr.binOpInstr(
        (command as es.BinaryExpression).operator,
        command as es.Node,
      ),
    );
  ...
  CallExpression: function (
    ...
    control.push(
      instr.appInstr(
        (command as es.CallExpression).arguments.length,
        command as es.CallExpression,
      ),
    );
    for (
      let index = (command as es.CallExpression).arguments.length - 1;
      index >= 0;
      index--
    ) {
  ...
  [InstrType.RESET]: function (
    ...
    if (
      cmdNext &&
      (isNode(cmdNext) || (cmdNext as Instr).instrType !== InstrType.MARKER)
    ) {
  ...
  [InstrType.UNARY_OP]: function (
    ...
    stash.push(
      evaluateUnaryExpression((command as UnOpInstr).symbol, argument),
    );
  },
  ...
  [InstrType.APPLICATION]: function (
      ...
      if (isSimpleFunction((func as Closure).node)) {
        ...
        control.push(
          returnStatement.argument ??
            identifier("undefined", returnStatement.loc),
        );
      } else {
        if (control.peek()) {
    ...
    let function_name = (
      ((command as AppInstr).srcNode as es.CallExpression)
        .callee as es.Identifier
    ).name;
  ...
  [InstrType.BRANCH]: function (
    ...
    if (test.value) {
      if (!valueProducing((command as BranchInstr).consequent)) {
        control.push(
          identifier("undefined", (command as BranchInstr).consequent.loc),
        );
      }
      ...
      if (!valueProducing((command as BranchInstr).alternate!)) {
        control.push(
          identifier("undefined", (command as BranchInstr).alternate!.loc),
        );
      }
      ...
      control.push(
        identifier("undefined", (command as BranchInstr).srcNode.loc),
      );
    }
  },
