...
export class Translator
  implements StmtNS.Visitor<BaseNode>, ExprNS.Visitor<BaseNode>
{
  ...
  private toEstreeLocation(stmt: Stmt | Expr): EstreeLocation {
    ...
    const source: string = this.source.slice(
  ...
  visitIfStmt(stmt: StmtNS.If): IfStatement {
    return {
      ...
      alternate:
        stmt.elseBlock !== null ? this.wrapInBlock(stmt, stmt.elseBlock) : null,
      loc: this.toEstreeLocation(stmt),
    };
  }
  ...
  visitCompareExpr(expr: ExprNS.Compare): BinaryExpression {
    ...
    switch (op) {
      ...
      case TokenType.NOTIN:
        throw new TranslatorErrors.UnsupportedOperator(
          expr.operator.line,
          expr.operator.col,
          this.source,
          expr.operator.indexInSource,
        );
      default:
        throw new Error("Unreachable binary code path in translator");
    }
