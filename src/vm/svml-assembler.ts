...
export function disassemble(p: Uint8Array): SVMProgram {
  ...
  for (let fnIdx = 0; fnIdx < functionOffsets.length; fnIdx++) {
    ...
    functions.push([stackSize, envSize, numArgs, instructions])
  ...
  for (const { fnIndex, instrIndex, targetOffset } of closureFixups) {
    ...
    const instruction = functions[fnIndex][3][instrIndex]
    instruction.arg1 = targetIndex
  }

  return [entrypointIndex, functions]
}
