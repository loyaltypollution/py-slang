...
function renameVariables(
  ...
  recursive(baseNode, new Set<string>(), {
    VariablePattern(node: es.Identifier, inactive: Set<string>, _c: (node: Node, state: Set<string>) => void) {
      // for declarations
      const name = node.name
    ...
    Identifier(node: es.Identifier, inactive: Set<string>, _c: (node: Node, state: Set<string>) => void) {
      // for lone references
      const name = node.name
      if (inactive.has(name)) {
        return
    ...
    BlockStatement(node: es.BlockStatement, inactive: Set<string>, c: (node: Node, state: Set<string>) => void) {
      if (baseScope) {
        baseScope = false
        for (const stmt of node.body) {
          c(stmt, inactive)
    ...
    IfStatement(node: es.IfStatement, inactive: Set<string>, c: (node: Node, state: Set<string>) => void) {
    ...
    Function(node: es.Function, inactive: Set<string>, c: (node: Node, state: Set<string>, type?: string) => void) {
...
function transformForLoopsToWhileLoops(program: es.Program) {
  simple(program, {
    ForStatement(node: es.Statement) {
