...
import { recursive, simple, pySlangBase } from "../utils/walkers";
...
function toCompileTaskBody(
    toCompileTask: CompileTask
): es.BlockStatement | es.Program {
...
function toCompileTaskIndexTable(
    toCompileTask: CompileTask
): Map<string, EnvEntry>[] {
...
function makeIndexTableWithPrimitivesAndInternals(
    ...
    for (let [index, [name]] of PRIMITIVE_FUNCTIONS) {
        names.set(name, { index, isVar: false, type: "primitive" });
    }
    ...
    recursive(
        ...
        new Set<string>(),
        {
            VariablePattern(
                node: Node,
                inactive: Set<string>,
                _c: (node: Node, state: Set<string>) => void
            ) {
            ...
            Function(
                node: Node,
                inactive: Set<string>,
                c: (node: Node, state: Set<string>, type?: string) => void
            ) {
                node = node as es.Function;
...
function compileArguments(exprs: Node[], indexTable: Map<string, EnvEntry>[]) {
    ...
    for (let i = 0; i < exprs.length; i++) {
        const { maxStackSize: curExpSize } = compile(
            exprs[i],
            indexTable,
            false
        );
...
function compileStatements(
    ...
    for (let i = 0; i < statements.length; i++) {
        if (
            ...
            loopTracker[loopTracker.length - 1][CONT_DEST_INDEX] =
                functionCode.length;
        }
...
const compilers = {
    ...
    CallExpression(
        ...
        if (node.callee.type !== "Identifier") {
            // ({ maxStackSize: maxStackOperator } = compile(
            //     node.callee,
            //     indexTable,
            //     false
            // ));
        ...
        // if callee == "__py_adder", emit the Add instruction instead
        if (callee.name === "__py_adder") {
            const { maxStackSize: m1 } = compile(node.arguments[0], indexTable, false);
            const { maxStackSize: m2 } = compile(node.arguments[1], indexTable, false);
            addNullaryInstruction(OpCodes.ADDG);
            return { maxStackSize: m1 + m2, insertFlag };
        }
        ...
        if (callType === "primitive") {
            ...
            addUnaryInstruction(
                isTailCallPosition ? OpCodes.CALLT : OpCodes.CALL,
                node.arguments.length
            );
            maxStackOperands++;
        }
        ...
        return {
            maxStackSize: Math.max(maxStackOperator, maxStackOperands, 1),
            insertFlag,
        };
    },
    ...
    ArrowFunctionExpression(
        ...
        const newSVMFunction: SVMFunction = [
            NaN,
            names.size,
            node.params.length,
            [],
        ...
        pushToCompile(
            makeToCompileTask(bodyNode, [functionIndex], extendedIndexTable)
        );
    ...
    Identifier(
        ...
        try {
            ...
            const errorName = (error as UndefinedVariable).name;
            // checking constant primitives
            console.log(CONSTANT_PRIMITIVES, node);
            const matches = CONSTANT_PRIMITIVES.filter(
                (f: [string, any]) => f[0] === errorName
            );
            if (matches.length === 0) {
                throw error;
            }
    ...
    Literal(
        ...
        if (value === null) {
            ...
            const value = Number(node.bigint);
            ...
            switch (typeof node.value) {
                case "boolean":
                    ...
                    break;
                case "number":
                    addUnaryInstruction(OpCodes.LGCF64, node.value);
                    break;
                case "string":
                    addUnaryInstruction(OpCodes.LGCS, node.value);
                    break;
                default:
    ...
    ArrayExpression(
        ...
        const elements = node.elements as (es.Expression | null)[];
        let maxStackSize = 1;
    ...
    ForStatement(
        _node: Node,
        _indexTable: Map<string, EnvEntry>[],
        _insertFlag: boolean
    ) {
        throw Error("Unsupported operation");
    },
    ...
    WhileStatement(
        ...
        const locals = extractAndRenameNames(
    ...
    BreakStatement(
        ...
        addNullaryInstruction(OpCodes.POPENV);
        loopTracker[loopTracker.length - 1][BREAK_INDEX].push(
            functionCode.length
        );
        addUnaryInstruction(OpCodes.BR, NaN);
    ...
    ContinueStatement(
        ...
        loopTracker[loopTracker.length - 1][CONT_INDEX].push(
            functionCode.length
        );
        addUnaryInstruction(OpCodes.BR, NaN);
        return { maxStackSize: 0, insertFlag };
...
function compile(
    ...
    insertFlag: boolean,
    isTailCallPosition: boolean = false
): { maxStackSize: number; insertFlag: boolean } {
    const compiler = compilers[expr.type as keyof typeof compilers] as any;
    if (!compiler) {
        throw Error("Unsupported operation");
    }
    const compilerResult = compiler(
        ...
        insertFlag,
        isTailCallPosition
    ) as { maxStackSize: number; insertFlag: boolean };
    const { maxStackSize: temp, insertFlag: newInsertFlag } = compilerResult;
...
export function compileToIns(
    ...
    const topFunctionIndex = prelude ? PRIMITIVE_FUNCTIONS.size + 1 : 0 // GE + # primitive func
        ...
        program,
        {
            ForStatement(node: Node) {
                ...
                if (init!.type === "VariableDeclaration") {
                    const loopVarName = (
                        (init as es.VariableDeclaration).declarations[0]
                            .id as es.Identifier
                    ).name;
                    ...
                    const innerBlock = create.blockStatement([
                        create.constantDeclaration(
                            loopVarName,
                            create.identifier(copyOfLoopVarName),
                            loc
                        ),
                        body,
                    ]);
                ...
                const newLoopBody = create.blockStatement([
                    forLoopBody,
                    assignment2,
                ]);
                const newLoop = create.whileStatement(
                    newLoopBody,
                    test!
                ) as taggedWhileStatement;
                newLoop.isFor = true;
                const newBlockBody = [assignment1, newLoop];
                (node as any).type = "BlockStatement";
                (node as any).body = newBlockBody;
            },
        },
        pySlangBase
    );
