 ...
 *  `definitelyBoundAnalysis.env.store.read(block).get(slot)`. A missing
 *  binding at the read site would indicate a seeding bug (the invariant is
 *  that every slot is always present); callers should treat that as an
 *  internal error rather than "unbound." */
export const definitelyBoundAnalysis: BlockFixpointAnalysis<BoundStatus> =
  makeBlockFixpointAnalysis<BoundStatus>({
    debugName: "definitelyBoundAnalysis",
    direction: "forward",
    mergeKind: "must",
