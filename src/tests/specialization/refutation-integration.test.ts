...
describe("refutation filter: worklist integration", () => {
  test("sibling-prefix chains carrying the refuted binding are retired", () => {
    // def f(x, y): ...
    // Run A: publish x=true, y=true → chain {p0:true, p1:true}.
    // Run B: under that chain, publish y=false → refutes p1:true.
...
`);
    ...
    worklist.publish(
      runtimeParamChannel, paramKey(fd.id, 1),
      { kind: "bool", value: false }, twoParamChain,
    );
    worklist.drain();

    // Sibling chain: only the refuted binding, no p0 prefix.
    const sibling = extend(
      ROOT_CONTEXT,
      p1Binding!.narrowing,
      p1Binding!.key,
      p1Binding!.value,
