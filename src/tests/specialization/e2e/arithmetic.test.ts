...
// Binary op specialization: monomorphic-int loop body must pick F-opcodes,
...
`;
  ...
  test.each([
    ["<", OpCodes.LTF, OpCodes.LTG, ascending, 10],
    ["<=", OpCodes.LEF, OpCodes.LEG, ascending, 10],
    ["!=", OpCodes.NEQF, OpCodes.NEQG, ascending, 10],
    [">", OpCodes.GTF, OpCodes.GTG, descending, 0],
    [">=", OpCodes.GEF, OpCodes.GEG, descending, 0],
  ] as const)(
    "guard %s selects %p, drops %p",
    (guard, specialized, generic, template, limit) => {
      const code = template.replace("GUARD", guard).replace("LIMIT", String(limit));
