...
function observeCallsTo(reactive: Worklist, fd: StmtNS.FunctionDef, n: number): void {
  for (let i = 0; i < n; i++) reactive.bump(runtimeCallCounter, fd.id);
}
...
describe("memoization: SVML wiring", () => {
  ...
  test("__memo_put writes to the shared slab", async () => {
    ...
    reactive.drain();
    // Python ints are bigint at runtime; the arg-key serializer tags them
    // with `typeof` so "bigint:5" is the expected form (not "number:5"
    // — which now means float).
...
`;
    ...
    // bucket has a single (bigint:5 → 6n) pair, not two.
