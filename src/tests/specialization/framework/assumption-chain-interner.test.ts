...
describe("interner reconvergence across widen → re-extend", () => {
  ...
  it("fact written before widen is readable after re-extending the pruned link", () => {
    ...
    const c1 = extendContext(ROOT_CONTEXT, nx, 1, 1);
    const c2 = extendContext(c1, ny, 2, 2);

    const store = new AnalysisStore<string, number>(intMax, undefined);
    store.write("k", 42, c2);

    const widened = excludeAssumption(c2, ny, 2);
    ...
    const c2Reborn = extendContext(widened, ny, 2, 2);
  ...
  it("order-independent observation reaches the same store partition", () => {
    ...
    // not two.
    const forward = extendContext(extendContext(ROOT_CONTEXT, nx, 1, 1), ny, 2, 2);
    const reverse = extendContext(extendContext(ROOT_CONTEXT, ny, 2, 2), nx, 1, 1);
